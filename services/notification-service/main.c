...
int main() {
    ...
    printf("notification-service rodando na porta %d\n", PORT);
    ...
    while (1) {
        ...
        // Se acessar /notifications, responde lista de notificações
        if (strstr(buffer, "GET /notifications")) {
            send_response(client, "{\"notifications\":[{\"id\":1,\"message\":\"Nova atividade criada\"},{\"id\":2,\"message\":\"Prazo de entrega proximo\"}]}");
        ...
        // Se acessar outra rota, responde que o serviço está online
            ...
            send_response(client, "{\"service\":\"notification-service\",\"status\":\"online\"}");
        }
