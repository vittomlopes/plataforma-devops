...
int main() {
    ...
    printf("task-service rodando na porta %d\n", PORT);
    ...
    while (1) {
        ...
        // Se acessar /tasks, responde lista de tarefas
        if (strstr(buffer, "GET /tasks")) {
            send_response(client, "{\"tasks\":[{\"id\":1,\"title\":\"Estudar DevOps\",\"status\":\"pendente\"},{\"id\":2,\"title\":\"Fazer Sprint 2\",\"status\":\"andamento\"}]}");
        ...
        // Se acessar outra rota, responde que o serviço está online
            ...
            send_response(client, "{\"service\":\"task-service\",\"status\":\"online\"}");
        }
